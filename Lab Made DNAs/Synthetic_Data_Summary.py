...
def process_fasta_file(filepath, filename):
    ...
    is_synthetic = 1 if ("lab_made" in filepath.lower() or "synthetic" in filename.lower() or "lab made" in filepath.lower()) else 0
    organism = "Lab-Engineered Construct" if is_synthetic else "Bacterial Genome"
    seq_type = "Synthetic Plasmid/Vector" if is_synthetic else "Natural Chromosome"
    ...
    for record in SeqIO.parse(filepath, "fasta"):
        ...
        homo_g = sum(1 for h in homopolymers if h[0] == 'G')  # Fixed: Added missing homo_g
        ...
        low_complexity_fraction = sum(len(m) for m in low_comp_matches) / length  # Fixed: removed extra 'h'
...
if __name__ == "__main__":
    synthetic_dir = r"C:\Users\vinay\OneDrive\Desktop\VMedithon\demo_samples\Lab Made DNAs" 
    
    synthetic_records = []
    
    if os.path.exists(synthetic_dir):
        files = sorted(
            [f for f in os.listdir(synthetic_dir) if f.startswith("synthetic_") and f.endswith(('.fasta', '.fa', '.txt'))],
            key=lambda x: int(re.search(r'\d+', x).group()) if re.search(r'\d+', x) else 0
        )
        
        for file in files:
            filepath = os.path.join(synthetic_dir, file)
            print(f"Extracting [Synthetic]: {file}")
            synthetic_records.extend(process_fasta_file(filepath, file))

        df_synthetic = pd.DataFrame(synthetic_records)
        output_csv = os.path.join(synthetic_dir, "synthetic_features.csv")
        df_synthetic.to_csv(output_csv, index=False)
        
        print(f"\nSUCCESS! Generated {output_csv}")
        print(f"Total Synthetic files processed: {len(df_synthetic)}")
        print(f"Total features extracted per file: {df_synthetic.shape[1]}")
    else:
        print(f"Directory non-existent: '{synthetic_dir}'. Please verify the path.")